const ripple = () => {
  document.addEventListener('click', (event) => {
    ...
    if (targetItem.closest('[data-ripple]')) {
      ...
      ripple.style.left = `${event.pageX - (button.getBoundingClientRect().left + scrollX) - radius
        }px`
      ripple.style.top = `${event.pageY - (button.getBoundingClientRect().top + scrollY) - radius
        }px`
